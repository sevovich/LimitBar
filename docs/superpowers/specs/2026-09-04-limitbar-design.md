...
The menu-bar label shows both providers at the same time. Each provider can show its short and long limit windows, for example `C 68/39 · A 53/27`, where the values are remaining percentages for the five-hour and weekly windows.
...
The tray label uses monospaced digits and compact provider initials. With both windows enabled, it renders `C short/long · A short/long`. If one window is disabled, each provider renders one number. Missing values render as an em dash and stale values keep their last number with a visible stale state in the popover.
